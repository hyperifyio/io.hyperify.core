...
import { explainUnitTypeOrUndefined, isUnitTypeOrUndefined, UnitType } from "../types/UnitType";
...
export function isAutoSizeType (value : unknown) : value is SpecialSize.AUTO {
...
export function isSizeDTO (value: unknown) : value is SizeDTO {
    return (
        ...
        && (isNumber(value?.value) || value?.value === SpecialSize.AUTO)
...
export function explainNumberOrAuto (value: unknown) : string {
    return isNumber(value) || value === SpecialSize.AUTO ? explainOk() : explainNot(explainOr(['number', '"auto"']));
}
